...
if ($IsAuthourized == 1) {
    ...
    // updated values of columns for that course
...
WHERE id = ?";
    ...
    // catch body will be executed if course tag is unique [unique constraint]
    try {
        $ErrorCheck = $Query->execute();
        if ($ErrorCheck) echo json_encode(array("message" => "Course Is Updated"));
        else echo json_encode(array("message" => "Failed To Update Course"));
    } catch (Exception $e) {
